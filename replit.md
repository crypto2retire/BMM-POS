...
- **Rent Tracking**: Monthly rent tracking for vendors, including payment methods and overdue flagging. Admin can record rent payments from the dashboard (Pay button on all vendor rows, or via detail panel). Payment modal supports cash/check/zelle/square/other with duplicate-month prevention.
- **Payout Processing**: Admin payout page (`/admin/payouts.html`) with preview and processing. Payouts automatically deduct monthly rent from vendor sales balances. If sales < rent, the shortfall becomes rent due. Vendors receive email notifications for payouts (with rent breakdown) or shortfall notices. Rent reminders can be sent manually: 15-day past-due and 27-day final notice emails. Endpoints: `GET /api/v1/admin/payout-preview`, `POST /api/v1/admin/process-payouts`, `POST /api/v1/admin/send-rent-reminders`. Payout model: `payouts` table with `gross_sales`, `rent_deducted`, `net_payout`, `status`, `period_month`.
...
- **Email Notifications**: Gmail API integration via Replit connector (`app/services/email.py`). Branded HTML email templates (`app/services/email_templates.py`) for: product sold, payout processed, expiring items, weekly reports, rent due, vendor welcome, order confirmations, order ready for pickup, and test emails. Notification toggles in admin settings control which emails are sent. Test email endpoint at `POST /api/v1/notifications/test-email`. **Customizable Email Templates**: Admin can edit subject, greeting, body, and closing for 12 email types via Settings > Notifications tab. Custom values stored as `email_tpl_{key}_{field}` settings (empty = use default from `EMAIL_TEMPLATE_DEFAULTS`). Templates support variable interpolation (e.g., `{vendor_name}`, `{item_name}`). Connected email account status shown with green/yellow/red indicator. Endpoints: `GET /api/v1/notifications/connected-email`, `GET /api/v1/notifications/email-templates`.
- **Online Order Ready-for-Pickup Notifications**: Online reservations include optional `customer_email` field. POS Online Orders panel has a "Notify Ready" button that transitions order status from `pending` → `ready` and sends a branded email with item name, store address, and store hours (pulled from `hours_*` settings). The "Ready" filter tab (blue) lets cashiers see orders awaiting pickup. Idempotent: calling ready on an already-ready order is a no-op. Endpoint: `POST /api/v1/admin/reports/reservations/{id}/ready`. Reservation statuses: `pending` → `ready` → `completed`.
- **Storefront Security**: Public storefront endpoints use UUID `public_id` for reservations (not sequential integers) to prevent ID enumeration. Tax rate on storefront is loaded dynamically from admin settings via `GET /api/v1/storefront/tax-rate` (consistent with POS `get_tax_rate()`). Rate limiting: 10 requests/minute/IP on `create-payment` and `payment-confirmed`. Input validation on customer name (1-200 chars), phone (7-50 chars), and email (200 chars max, `@` required). Login brute-force protection: 10 attempts/IP/5 minutes.
- **Vendor Booth Showcase**: Vendors can create a public booth profile with photos (1-8, required for publishing), optional video (MP4/MOV/WebM, up to 50MB), description (AI-assisted via OpenRouter), and booth name. Published showcases appear on the public "Booths" page (`/shop/booths.html`) with a photo gallery, video player, and booth number badge. The 60-day stale photo reminder shows on both the vendor dashboard and booth showcase page when photos are overdue for refresh. Model: `booth_showcases` table (one per vendor). Routes: `app/routers/booth_showcase.py`. Vendor page: `/vendor/booth-showcase.html`. Public page: `/shop/booths.html`. Navigation: "Booths" tab in shop header nav, "My Booth" in vendor nav/bottom nav.
- **Admin Data Reset**: Full database reset endpoint (`POST /api/v1/admin/reset-data`) clears all vendors (except admin/cashier), items, sales, gift cards, reservations, studio classes, payouts, rent payments, booth showcases, and balance adjustments. Preserves admin/cashier accounts and store settings. Requires typing "CLEAR ALL DATA" to confirm. UI button in Admin > Data Import section.
