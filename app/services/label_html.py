...
def generate_label_html(items, label_width_in: float = 1.5, label_height_in: float = 1.0) -> str:
    ...
    css = (
        ...
        ".divider { border-top: 0.5pt solid #666; margin: 0.01in 0; }"
        ...
        ".barcode { width: 100%; height: auto; max-height: 0.26in; display: block; margin-top: 0.01in; }"
        ".barcode-text { font-size: 5pt; text-align: center; letter-spacing: 0.02em; line-height: 1; }"
        "@media screen {"
    ...
    script = (
        ...
        "if(!val)return;"
        "try{JsBarcode(svg,val,{format:'CODE128',width:1.4,height:22,"
