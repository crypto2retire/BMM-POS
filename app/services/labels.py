...
def _draw_label(c, item, x_offset, y_offset, w=None, h=None):
    ...
    if barcode_val:
        ...
        # Code128 requires quiet zone >= 10× module width. At 3-dot modules
        # on small labels, that's 30 dots (0.1"). Larger labels get 20 dots.
        quiet_zone = (30 if small_label else 20) * DOT
...
def generate_dymo_xml(item, label_size: str = None) -> str:
    ...
    if label_size == "30347" and _supports_small_label_code39(raw_barcode):
...
"""
  ...
  <ObjectInfo>
    ...
    if barcode_str:
        ...
        dymo_quiet_zone = "0" if small_dymo_uses_code39 else "200"
        xml += f"""
  <ObjectInfo>
    <BarcodeObject>
      <Name>BARCODE</Name>
